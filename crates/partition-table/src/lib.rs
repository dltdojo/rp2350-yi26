...
//! ```
//!
//! # What is independently confirmed — now including the one thing that was not
//!
//! The values here were mirrored from `embassy-rp`'s encoder and then checked
//! against the Pico SDK's own `picobin.h`. Twelve of them agreed exactly: both
//! markers, all six permission bits, and all six family bits. **The thirteenth
//! — the size field, the one that decides whether a board boots — was the
//! outlier, and it is no longer.**
//!
...
//! (`ITEM_1BS_IMAGE_TYPE`, `ITEM_2BS_LAST`), so the width had to be inferred.
//! The second source turned out to be sitting in `embassy-rp`'s own test suite:
//! a table generated by `picotool partition create` has the header word
//! `0x02000c0a`, which decodes only as the two-byte layout — count `2` in the
//! top byte, length `12` in bits 8..23, id `0x0a`. The one-partition table this
//! crate emits uses the same layout, `0x0100040a` (count `1`, length `4`). So
//! `picotool` and `embassy-rp` agree on the width, and the whole eight-word
//! block this crate produces is byte-for-byte `embassy-rp`'s own
//! `PartitionTableBlock::add_partition_item` output for a minimal partition. See
//! [`SIZE_FIELD_IS_UNCONFIRMED`], kept only to record that the question is now
//! settled.
//!
...
/// The size field, once the one value without a second source — now confirmed,
/// so this is `false`. Kept, rather than deleted, so the record reads forward:
/// what was uncertain, and what settled it.
///
...
/// 0x0001040a   one-byte size — the alternative, now ruled out
/// ```
///
/// The two-byte reading is confirmed twice over. Structurally it is the
/// likelier one — a partition table can hold sixteen partitions of several
/// words each, and a one-byte length caps an item at 255 words — but it no
/// longer rests on that. A table generated by `picotool partition create` (in
/// `embassy-rp`'s own test) has the header `0x02000c0a`, which decodes only as
/// two-byte size, and `embassy-rp`'s encoder emits the same layout. Two
/// independent sources agree.
///
/// **If a board flashed with a table from this crate does not boot, do not
/// start here.** The old advice — flip this word to
/// [`ONE_BYTE_SIZE_ALTERNATIVE`] — is superseded. exp139 showed the ROM honours
/// this table (the drive refused writes, which a rejected table would not
/// cause); look at the image-in-partition setup and the flash path instead.
pub const SIZE_FIELD_IS_UNCONFIRMED: bool = false;

/// The first item word a one-byte size field *would* produce — the alternative
/// this crate ruled out. See [`SIZE_FIELD_IS_UNCONFIRMED`]. Kept so the
...
mod tests {
    ...
    ///
    /// `embassy-rp`'s test suite embeds a table generated by
    /// `picotool partition create`. Its partition-table item header is
    /// `0x02000c0a`, and it decodes only as the two-byte layout `item(count,
    /// length, id)` — two partitions, twelve words (the header plus its body),
    /// id `0x0a`. This crate's one-partition header is the same layout with
    /// different numbers. So `picotool` and this crate agree on the width, which
    /// is what [`SIZE_FIELD_IS_UNCONFIRMED`] being `false` records.
    #[test]
    fn the_size_field_has_a_second_source_now() {
        // picotool's generated header, from embassy-rp's `make_hashed_partition_table`.
        assert_eq!(item(2, 12, ITEM_PARTITION_TABLE), 0x0200_0c0a);
        // exp139's header — the same two-byte layout, one partition, four words.
        assert_eq!(item(1, 4, ITEM_PARTITION_TABLE), 0x0100_040a);
        // Same id, same layout; only count and length differ.
        assert_eq!(0x0200_0c0a & 0xff, ITEM_PARTITION_TABLE as u32);
        assert_eq!(0x0100_040a & 0xff, ITEM_PARTITION_TABLE as u32);
        // And the outlier is no longer flagged as unconfirmed.
        assert!(!SIZE_FIELD_IS_UNCONFIRMED);
    }
}
