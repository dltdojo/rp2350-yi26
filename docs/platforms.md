...
#### ...and a browser can reach it, under two conditions

Measured with [exp150](../experiments/exp150-a-page-served-by-the-board/) on
2026-08-05, after a first attempt that failed and a second that did not.

**It does not work with the board as the DHCP server.** Board on a static
`192.168.7.1`, Chrome at that address: `ERR_TIMED_OUT`, and the board records
`0 request(s) served` for 245 seconds. Tested with and without a DHCP router
option; identical.

**It works when the phone hands out the address.** Turn on Android's **Ethernet
tethering** (Settings → Network & internet → Hotspot & tethering — greyed until
a device is attached), build the board as a DHCP *client*, and the phone becomes
the server and the router. The board came up on `10.206.115.122` with a gateway
and its page rendered in Chrome, on the phone.

**And only by navigating to it.** From a `content://` page, `fetch()` is refused
before it leaves and an `<iframe>` stays blank — pulling an `http://` resource
into a secure page is **mixed content**. A top-level navigation is not mixing
anything into anything, so it goes. No number of CORS headers on the board
changes that; it is not a CORS decision.

So the rule for a phone is narrow and worth stating exactly:

> **The address has to be the phone's to give, and the page has to be navigated
> to rather than fetched.**

Mobile data survived every variant throughout, including a build that announced
the board as the gateway. A USB Ethernet gadget on this phone displaces nothing.
