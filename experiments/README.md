...
| [exp195-the-bug-the-model-saw-first](./exp195-the-bug-the-model-saw-first/) | 1 · board | **Under construction.** A model checker asked the questions a board answered at the cost of a bench run: `crates/breadcrumb` before and after its tag fix, and `crates/usb-log` under two schedulers with its predictions committed before the model was ever run |
...
- **a credential nobody asked for** — `makeCredential`'s other five parts.
  [exp171](./exp171-a-credential-nobody-asked-for/) is **verified on hardware**:
  a real WebAuthn credential, P-256, self-attested, checked off the board by a
  different elliptic-curve library with a bit flipped and the check required to
  fail first. **44.5 ms to derive the key, 53.8 ms to sign**, 276 bytes of
  response.

  **The private key is derived and never stored** — from the device secret, a
  TRNG nonce and the relying party — so nothing is in flash to find and exp163's
  exposure measurement applies unchanged to the 98 ms it exists.

  Its name is its finding. **The bit that says a person was present is the
  device's own word and nothing in the protocol checks it**, so user presence is
  a build input and both settings are honest: one asks nobody and reports `UP=0`,
  one waits for BOOTSEL. Neither ever sets the bit without earning it, and
  `check.sh` fails if the unattended build's bit is ever 1. **Both branches are
  on the board**: `flags = 0x40` with nobody asked, and `0x41` with a finger on
  BOOTSEL — and the press is checked in as its own transcript, because a script
  cannot press a button and can check that somebody did.

  The device secret is a compiled-in test key whose thirty-two bytes spell *not
  a secret. this is a test key*, so exp166's byte search finds a sentence. That
  is the [identity road](#the-identity-road) arriving with a name. Needs 2 —
  everything but one transcript runs unattended, and that one needs a finger.

- **the same key twice** — `authenticatorGetAssertion`.
  [exp172](./exp172-the-same-key-twice/) is **verified on hardware**: the board
  re-derives a private key **it never stored**, from forty-eight bytes handed
  back to it, and the assertion **verifies against the public key registration
  gave out**. Registration and assertion are minutes apart and cost 44,460 and
  44,448 µs to derive — twelve microseconds, because it is the same work.

  **The credential ID is not a pointer into a table; there is no table.** Its
  tag is `HMAC(secret, "id" ‖ nonce ‖ rpIdHash)`, so recomputing it answers both
  questions at once: did this device make you, and for this relying party. A
  forged tag, a credential from another site and one with no tag at all are all
  refused — **before anything is derived**, because the status byte is the same
  either way and the difference is whether an attacker got the work done.

  The tag comparison accumulates rather than returning early, and `check.sh`
...
- **the same question, asked of two devices** — [exp176](./exp176-the-same-question-of-two-devices/)
  is **verified on hardware** against a Yubico Security Key. `fido2-token -I` and
  one registration each, with every difference sorted by kind: **ten of fourteen
  are code the board could write** — `rk`, `hmac-secret`, `credProtect`, `eddsa`,
  `clientPin` and the rest — **one is certification the chip cannot anchor**, the
  real AAGUID and the attestation certificate behind it. That one is exp175's gap
  exactly: a secret the image carries is a secret anyone with the image has. The
  interrogation's warning held — the categorisation is a claim, so it is written
