...
async fn http_task(
    ...
    loop {
        ...
        let (status, ctype, len) = match http_route::parse(&req[..have]) {
            ...
            // Parsed, well-formed, and names nothing. A 404 and not a 400 —
            // the difference matters to whoever is reading the log, because one
            // of them means "you asked for something that is not here" and the
            // other means "I could not tell what you asked".
            Parsed::Complete(Request { route: Route::Unknown, path, .. }) => {
            ...
            // A method this board answers on a route that exists — POST /log.
            // Nothing here writes, which is exp155's subject, so this is a 405
            // rather than a quiet success.
            Parsed::Complete(Request { method: Method::Post, .. }) => {
                COUNT_REFUSED.fetch_add(1, Ordering::Relaxed);
