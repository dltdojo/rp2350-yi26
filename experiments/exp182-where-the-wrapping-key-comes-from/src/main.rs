...
//!
//! `EXP182_HOLD_MS` is that floor, `EXP182_TIMEOUT_MS` is the window, and
//! `EXP182_KEEPALIVE` is the arm. At `HOLD_MS=0, KEEPALIVE=off` this is exp173.
...
include!(concat!(env!("OUT_DIR"), "/exp182_config.rs"));
...
/// Where this device's secret comes from, and why the previous one had to go.
///
/// [exp171](../exp171-a-credential-nobody-asked-for/) through
/// [exp174](../exp174-a-deadline-nobody-mentioned/) used a compiled-in constant
/// whose thirty-two bytes spelled *not a secret. this is a test key*, and said
/// so on every line of their logs. [exp175](../exp175-the-secret-is-the-file/)
/// then showed exactly what that costs: `forge.py` finds those bytes in the
/// `.uf2` with a search and mints a working WebAuthn assertion, no board
/// involved. Possession of the image was possession of the identity.
///
/// This build takes the secret from
/// [exp181](../exp181-a-key-that-is-written-nowhere/) instead: a 256-bit key
/// reconstructed at boot from SRAM bank 8, whose startup pattern
/// [exp179](../exp179-what-survives-a-reset/) measured surviving power-on. What
/// is in flash is `H = K ⊕ w`; what is in the image is nothing.
///
/// **Two consequences the earlier builds never had.** A board straight from
/// `yi26 flash` is *unprovisioned* — exp179 measured that flashing zeroes SRAM,
/// so there is nothing to enrol from until the power has been away once. And a
...
const _: () = assert!(
    ...
    "EXP182_HOLD_MS must be under the presence timeout, or no press can ever land"
);
...
/// **A different refusal needs a different number.**
///
/// The first version of this firmware answered an unprovisioned board with
/// `CTAP2_ERR_OPERATION_DENIED`, which is also what it answers when nobody
/// presses the button. The two are not the same fact — one is *come back after
/// a power cycle*, the other is *you were not there* — and
/// [exp173](../exp173-a-client-that-is-not-ours/) is an entire experiment about
/// what one shared number costs: `FIDO_ERR_INVALID_PARAM` meant "your user was
/// not present" and read as "your attestation is malformed", and five
/// experiments walked past it.
///
/// CTAP 2.1 has no status for *this device has no key material yet*, so this is
/// the first vendor code rather than a standard one bent to fit. A client that
/// does not know it sees a number and has to ask the device — which is honest,
/// and is what exp169 established about claiming things you do not have.
const CTAP2_ERR_VENDOR_UNPROVISIONED: u8 = 0xF0;
...
fn mac(secret: &[u8; 32], label: &[u8], nonce: &[u8], rp_id_hash: &[u8], out: &mut [u8]) {
    let mut m = <Hmac as Mac>::new_from_slice(secret).unwrap();
    m.update(label);
    m.update(nonce);
    m.update(rp_id_hash);
...
fn derive_key(secret: &[u8; 32], nonce: &[u8], rp_id_hash: &[u8]) -> Option<SigningKey> {
...
fn build_credential(
    ...
    mac(secret, b"id", nonce_part, &rp_id_hash, &mut tag_part[..TAG_LEN]);

    let t0 = Instant::now();
    let sk = derive_key(secret, nonce, &rp_id_hash).ok_or(())?;
...
fn credential_is_ours(secret: &[u8; 32], cred_id: &[u8], rp_id_hash: &[u8; 32]) -> bool {
    if cred_id.len() != CRED_ID_LEN {
        return false;
    }
    let (nonce, tag) = cred_id.split_at(32);
    let mut want = [0u8; TAG_LEN];
    mac(secret, b"id", nonce, rp_id_hash, &mut want);
    tags_equal(&want, tag)
}
...
/// What the LED is saying, because it is the only channel that reaches
/// somebody who is not sitting at this host.
///
/// **This experiment printed its instructions to a terminal and they did not
/// arrive.** The board was being driven from another machine, where a script's
/// stdout is not something the person holding the board can see — and neither
/// is the firmware's log, which is also read on the host. `AGENTS.md` has the
/// rule already: *the LED is the debug channel, so design it before you need
/// it*, and [exp180](../exp180-the-silicon-or-the-room/) had already used three
/// LED states for exactly this. This one went back to words and cost a round
/// trip to find out.
const LED_IDLE: u8 = 0;
...
/// - **solid on** — press and hold BOOTSEL *now*; a request is waiting
/// - **two quick flashes, then a pause** — no secret: unplug it and plug it
///   back in, and nothing else will help
#[embassy_executor::task]
...
/// Where the reconstructed secret lives once, for as long as the board is
/// powered. Not a `static mut`: the borrow checker should be able to see that
/// nothing writes it twice, and `StaticCell` is how that is said.
static SECRET: StaticCell<[u8; 32]> = StaticCell::new();
...
async fn wait_for_presence(
    ...
    // Solid, for exactly as long as a press would count. Set here rather than
    // at the call sites so that no future caller can open a window silently.
    ...
    loop {
        ...
        if pressed_at.is_some() && start.elapsed().as_millis() >= HOLD_MS {
            LED_MODE.store(LED_IDLE, Ordering::Relaxed);
            return Waited {
...
async fn ctaphid_task(
    ...
    // `None` when this board has not been provisioned, or when the key did not
    // come back. Every operation that needs a secret refuses rather than signing
    // with something that is nearly right.
    secret: Option<&'static [u8; 32]>,
) -> ! {
                    ...
                    CTAPHID_CBOR => {
                        ...
                        match ctap {
                            ...
                            AUTHENTICATOR_MAKE_CREDENTIAL => {
                                // **An unprovisioned board has no secret and
                                // refuses.** Not a placeholder key, not zeros:
                                // signing with something that is nearly right
                                // hands out credentials nobody can check, and
                                // exp175's whole point is what happens when the
                                // secret is somewhere it should not be. The
                                // status is the one exp170 chose — understood,
                                // and denied.
                                let Some(secret) = secret else {
                                    ERRORS.fetch_add(1, Ordering::Relaxed);
                                    log!("  refused: this board has no secret yet — power it off and on once");
                                    Timer::after(PACE).await;
                                    send(&mut writer, cid, CTAPHID_CBOR, &[CTAP2_ERR_VENDOR_UNPROVISIONED]).await;
                                    continue;
                                };
                                ...
                                match parse_get_assertion(body) {
                                    ...
                                    Ok(req) => {
                                        ...
                                        for c in &req.allow[..req.n_allow] {
                                            let ours = credential_is_ours(secret, c, &rp_id_hash);
                                            log!(
                                                "    credential {} bytes: {}",
                                                c.len(),
...
async fn main(spawner: Spawner) {
    // **First**, before any peripheral is touched. Bank 8 needs no clocks, and
    // whatever is in it now is what power left there; anything this firmware
    // does afterwards is a chance to disturb it.
    ...
    // Enrol or reconstruct, before anything can ask for a credential.
    ...
    match puf_read_record() {
        None => {
            if !(UNIFORMITY_MIN..=UNIFORMITY_MAX).contains(&uniformity) {
                // exp179's trap: on a just-flashed board the window is zero, and
                // `H = K ⊕ 0` is the key itself, sitting in flash. Refusing here
                // is the difference between this experiment and exp175's subject.
                provision.state = "UNPROVISIONED — refused to enrol on a cleared window";
            } else {
                let mut key = [0u8; 32];
                ...
                let bytes: &[u8] = unsafe {
                    core::slice::from_raw_parts(
                        &record as *const PufRecord as *const u8,
                        core::mem::size_of::<PufRecord>(),
                    )
                };
                ...
                if ok {
                    provision.enrolled_at = uniformity;
                    provision.secret = Some(&*SECRET.init(key));
                    provision.state = "enrolled from SRAM on this boot";
                } else {
                    provision.state = "UNPROVISIONED — the helper could not be written";
                }
            }
        }
        Some(r) => {
            ...
            if fc::hash(&key) == r.key_hash {
                ...
                // Not a near miss. A different secret is a different device, and
                // signing with one would hand out credentials nobody can check.
                provision.state = "UNPROVISIONED — the key did NOT come back";
            }
        }
    }

    // The LED is how somebody who cannot see this log finds out that the board
    // needs a power cycle before it can do anything.
    if provision.secret.is_none() {
        LED_MODE.store(LED_UNPROVISIONED, Ordering::Relaxed);
    }

    spawner.spawn(
        ctaphid_task(
            hid,
            TRANSACTION.init(Transaction::none()),
            trng,
            provision.secret,
        )
        .unwrap(),
    ...
    Timer::after(PACE).await;
    log!("  device secret: {}", provision.state);
    Timer::after(PACE).await;
    log!(
        "    bank 8 came up {}.{}% one-bits{}",
        provision.uniformity / 10, provision.uniformity % 10,
        if provision.uniformity == 0 { ", which is cleared — this board was just flashed (exp179)" } else { "" }
    );
    Timer::after(PACE).await;
    if provision.enrolled_at > 0 {
        log!(
    ...
    log!("    nothing in this image is the key; forge.py has nothing to lift (exp175)");
    Timer::after(PACE).await;
    {
    ...
    loop {
        ...
        // **Where the secret came from, every time, not once at boot.**
        //
        // exp180 lost a whole run to printing its result once and having it
        // scroll past an unattached host; this experiment inherited exp174's
        // report-once header and did it again, on the one line that says whether
        // the board can do anything at all. For somebody driving this from
        // another machine it is the only place that answer appears in words.
        log!("  device secret: {}", provision.state);
        Timer::after(PACE).await;
        if provision.enrolled_at > 0 {
            log!(
