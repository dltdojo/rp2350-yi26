...
//!
//! Implements CTAP 2.1 PIN Protocol 1:
//! 1. Key Agreement (ECDH P-256)
//! 2. Shared Secret derivation (SHA-256 of ECDH raw secret bytes)
//! 3. AES-256-CBC encryption/decryption tunnel with zero-IV
//! 4. HMAC-SHA256 (16-byte truncated) pinAuth validation
...
include!(concat!(env!("OUT_DIR"), "/exp185_config.rs"));
...
const PRODUCT: &str = "exp185 a channel before a secret";
                        ...
                        match ctap {
                            ...
                            AUTHENTICATOR_CLIENT_PIN => {
                                ...
                                match sub_cmd {
                                    ...
                                    Some(0x03) => {
                                        log!("  clientPIN: setPIN / tunnel verify request received");
                                        if pin_proto != Some(1) {
                                        ...
                                        // 1. Decapsulate shared secret via P-256 ECDH
                                        let shared_secret = match decapsulate_shared_secret(&session_sk, &peer_x, &peer_y) {
                                            Ok(s) => s,
                                            Err(_) => {
                                                log!("  ECDH decapsulation failed");
                                                send(&mut writer, cid, CTAPHID_CBOR, &[CTAP2_ERR_INVALID_CBOR]).await;
                                                continue;
                                            }
                                        };
                                        log!("  ECDH sharedSecret derived: {:02x?}...", &shared_secret[..8]);

                                        // 2. Verify pinAuth: HMAC-SHA256(sharedSecret, newPinEnc)[..16]
                                        if !verify_pin_auth(&shared_secret, enc, &auth) {
                                            log!("  pinAuth verification FAILED (signature invalid)");
                                            send(&mut writer, cid, CTAPHID_CBOR, &[CTAP2_ERR_PIN_AUTH_INVALID]).await;
                                            continue;
                                        }
                                        log!("  pinAuth PASS (HMAC valid)");

                                        // 3. Decrypt ciphertext using AES-256-CBC (IV=0)
                                        let mut decrypted = [0u8; 128];
                                        let dec_len = match decrypt_pin_payload(&shared_secret, enc, &mut decrypted) {
                                            Ok(l) => l,
                                            Err(_) => {
                                                log!("  AES-256-CBC decryption failed");
                                                send(&mut writer, cid, CTAPHID_CBOR, &[CTAP2_ERR_INVALID_CBOR]).await;
                                                continue;
                                            }
                                        };
                                        log!("  decrypted {}B payload: {:02x?}", dec_len, &decrypted[..dec_len.min(16)]);

                                        // Return OK with empty CBOR map
                                        out[0] = CTAP2_OK;
                                        let mut w = cbor::Writer::new(&mut out[1..]);
                                        w.map(0);
                                        w.end();
...
async fn main(spawner: Spawner) {
    ...
    log!("exp185 a channel before a secret");
    Timer::after(PACE).await;
    log!("  PIN Protocol 1 (ECDH P-256, AES-256-CBC, HMAC-SHA256)");
    Timer::after(PACE).await;
