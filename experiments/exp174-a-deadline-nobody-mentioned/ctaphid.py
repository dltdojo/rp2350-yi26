...
# exp174's two. The numbers are the command, without the initialisation bit
# the packet carries: KEEPALIVE goes out as 0xBB and CANCEL as 0x91.
CTAPHID_CANCEL, CTAPHID_KEEPALIVE = 0x11, 0x3B
...
class Link:
    ...
    def drain(self):
        ...
        **New in exp174, and new because of exp174.** Every earlier experiment
        here had a device that only ever spoke when spoken to, so a fresh
        client could assume an empty pipe. A device that sends `KEEPALIVE`
        while it waits does not: a run that ends during a presence wait leaves
        packets in the kernel's buffer, and the next `INIT` reads one of them
        as its own reply. The first version of this file did exactly that and
        failed with a missing channel id.
        """
        n = 0
        while True:
...
def main():
    ...
    case = sys.argv[1]
    link = Link()
    # See `Link.drain`. A device that speaks unprompted makes an empty pipe an
    # assumption rather than a fact, and this is where the fact is established.
    stale = link.drain()
    out = {"case": case, "device": link.path}
    if stale:
        out["stale_packets_discarded"] = stale
    ...
    elif case in ("keepalive", "cancel"):
        # **Both halves of exp174, and neither needs a finger.**
        #
        # A `button` build that has just been asked for a credential is sitting
        # in its presence wait, which is exactly the state this experiment is
        # about. So a client can count what the device sends while nobody
        # presses anything, and then withdraw the request and see what comes
        # back. `keepalive` watches; `cancel` watches and then leaves.
        watch = float(sys.argv[2]) if len(sys.argv) > 2 else 1.5
        r = init(link)
        cid = bytes.fromhex(r["new_cid"])
        link.send_message(cid, CTAPHID_CBOR,
                          bytes([0x01]) + make_credential_request())
        seen, other, end = 0, [], time.time() + watch
        final = None
        while time.time() < end:
            pkt = link.read_packet(timeout=0.2)
            if pkt is None:
                continue
            if pkt[4] == 0xBB:
                seen += 1
                ...
                other.append(pkt[4])
                # An answer arrived while we were counting: somebody pressed
                # the button, or the build does not wait at all.
                final = pkt
                break
        out["watched_s"] = watch
        out["keepalives_seen"] = seen
        out["other_packets"] = [hex(b) for b in other]
        # One byte a second, near enough, is the specification's floor; this
        # device aims at ten. Reported rather than asserted, because a host
        # that is slow to poll sees fewer than were sent.
        out["keepalives_per_s"] = round(seen / watch, 1) if watch else 0

        if case == "cancel" and final is None:
            link.send_message(cid, CTAPHID_CANCEL, b"")
            reply = link.read_message(timeout=3.0)
            out["reply"] = reply
            if reply and reply.get("cmd") == CTAPHID_CBOR:
                out["status"] = link.last[0]
                out["status_is_keepalive_cancel"] = link.last[0] == 0x2D
