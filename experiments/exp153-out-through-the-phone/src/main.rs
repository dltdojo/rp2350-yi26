//! exp153 — out through the phone.
//!
//! Every experiment on the network road so far has the board being *reached*.
//! This one points the same stack the other way and asks whether the board can
//! reach anything itself.
//!
//! # The claim being tested is one this repository wrote down and did not check
//!
//! The plan for this experiment, in `experiments/README.md`, says it is
//! **desktop only, and deliberately so**:
//!
//! > It needs the host to route and NAT for it, which a laptop can be told to
//! > do and a phone cannot — there is no such UI.
//!
//! That was written before [exp150](../../exp150-a-page-served-by-the-board/)
//! measured what Android's **Ethernet tethering** does. Tethering is not a
//! setting that gives out addresses; it is a setting that *shares a connection*
//! — the phone becomes the DHCP server, the router **and the NAT**. exp150 saw
//! the board come up with a gateway and never asked what was on the other side
//! of it.
//!
//! So the UI the plan says does not exist is the switch exp150, exp151 and
//! exp152 all already require somebody to turn on.
//!
//! # Three measurements, all of them free once the board is plugged in
//!
//! 1. **What was offered.** The lease's gateway and DNS servers, logged as
//!    received. A gateway is a claim that there is a way out; it is not proof.
//! 2. **Whether the way out goes anywhere.** A TCP connection to a literal
//!    address — no DNS in the path — and one HTTP request over it, twice:
//!
//!    | request | what it should answer |
//!    | --- | --- |
//!    | `GET /` with `Host: 1.1.1.1` | `301`, to `https://` |
//!    | `GET /generate_204` with `Host: cp.cloudflare.com` | `204 No Content` |
//!
//!    Both go to the same address, so a difference between them cannot be a
//!    routing difference. **The difference between those two answers is the
//!    finding**: a device that cannot speak TLS is redirected off almost
//!    everything, and what it can still have is the captive-portal endpoint —
//!    a URL that exists precisely because it must work before a network is
//!    trusted, and is therefore the last plain-HTTP thing on the internet.
//!
//! 3. **Whether a name can be turned into an address.** One DNS query, using
//!    the resolver the lease named. Reported separately and never in the path
//!    of measurement 2, because a client that fails at resolution and a client
//!    that fails at routing look identical from a bench and are not the same
//!    failure.
//!
//! # A literal address, on purpose, and it will still be there in a year
//!
//! `1.1.1.1` is anycast, it is the shortest address anybody can check by hand,
//! and it serves both URLs above on port 80. An experiment here is **frozen
//! once made**, so a target that can be renumbered is a walkthrough that will
//! one day describe something that no longer happens. This one names an address
//! rather than a name, and gets its two answers from a single operator.
//!
//! # What was taken out of exp152, and why
//!
//! exp152 **pins** itself to host `.250` of whatever subnet it is given, so a
//! bookmark outlives a lease. That is right for a board being reached and wrong
//! for a board reaching out: a NAT translates the addresses it handed out, and
//! sending from one it never leased would confound the only question this
//! experiment asks. So exp153 keeps the address it was given, and the drive
//! points at that.
...
// TCP both ways: four sockets that are accepted on, and one that connects. The
// same type, and the difference between the two directions is one call.
use embassy_net::tcp::TcpSocket;
use embassy_net::{Config as NetConfig, Ipv4Address, Runner as NetRunner, StackResources};
...
/// Where the board tries to go. See the module header for why this is a literal
/// address and why it is this one.
const OUT_HOST: [u8; 4] = [1, 1, 1, 1];
const OUT_PORT: u16 = 80;

/// The two requests, and they differ in one header. Sent to the same address
/// over separate connections, so the only variable between them is the name the
/// server is asked to answer as.
const OUT_ROOT: &str = "GET / HTTP/1.0\r\nHost: 1.1.1.1\r\nConnection: close\r\n\r\n";
const OUT_204: &str =
    "GET /generate_204 HTTP/1.0\r\nHost: cp.cloudflare.com\r\nConnection: close\r\n\r\n";

/// A name to resolve. Chosen as the one the second request already names, so a
/// resolver that works can be seen to agree with the literal address.
const OUT_NAME: &str = "cp.cloudflare.com";

/// How long to wait for the whole of one exchange. Generous: this crosses a
/// phone, a carrier and the public internet, and a timeout that fires on a slow
/// path would be recorded as a wall that is not there.
const OUT_TIMEOUT: Duration = Duration::from_secs(10);

/// How many times to try before giving up, and the gap between tries. A single
/// shot makes a transient failure cost a trip to the bench; an unbounded retry
/// loop fills the log with the reader's own waiting.
const OUT_ATTEMPTS: u32 = 3;
const OUT_RETRY_GAP: Duration = Duration::from_secs(5);

/// The status codes the two requests came back with, `0` until each is answered
/// and `1` if the connection itself never got anywhere. The page prints both,
/// because the pair is the result — either one alone is half of it.
static STATUS_ROOT: AtomicU32 = AtomicU32::new(0);
static STATUS_204: AtomicU32 = AtomicU32::new(0);

/// The address `OUT_NAME` resolved to, packed big-endian, or `0` for "no answer
/// yet or none". Separate from the two above on purpose: see the header.
static RESOLVED: AtomicU32 = AtomicU32::new(0);

/// Set once the attempts are over, however they went. Without it the page
/// cannot tell "still trying" from "tried and got nothing", and those are the
/// two readings somebody would act on differently.
static OUT_DONE: AtomicBool = AtomicBool::new(false);
...
/// Reads the lease out loud and hands the address to the disk.
///
/// exp152 did more here — it moved to a pinned host address so a bookmark would
/// outlive the lease. That is removed rather than kept and ignored, because a
/// NAT translates the addresses it handed out and this experiment's whole
/// question is what happens to a packet on the way out. Sending from an address
/// the server never leased would be a second variable in a one-variable
/// measurement.
///
/// The gateway and the DNS servers are logged **as received**, before anything
/// is tried with them. What a host offered and what a host will do are separate
/// facts, and a log that only ever prints the second one cannot tell an offer
/// that was never made from an offer that did not work.
#[embassy_executor::task]
async fn lease_task(stack: embassy_net::Stack<'static>) -> ! {
    ...
    log!(
        "lease: {}.{}.{}.{}/{}",
        got[0], got[1], got[2], got[3], cfg.address.prefix_len()
    );
    match cfg.gateway {
        Some(g) => {
            let g = g.octets();
            log!("  gateway {}.{}.{}.{} — a claim that there is a way out", g[0], g[1], g[2], g[3]);
        }
        None => log!("  NO gateway offered — this link goes nowhere but the host"),
    }
    if cfg.dns_servers.is_empty() {
        log!("  no DNS server offered — a name cannot become an address here");
    } else {
        for s in cfg.dns_servers.iter() {
            let s = s.octets();
            log!("  DNS {}.{}.{}.{}", s[0], s[1], s[2], s[3]);
        }
    }
...
/// One line of the result box: a request, and what came back.
///
/// The three readings are kept distinct because they are acted on differently.
/// "still trying" asks somebody to wait; "no answer" asks them to look at the
/// tethering switch; a number ends the experiment.
fn outcome(w: &mut Cursor, what: &str, code: u32) {
    let _ = write!(w, "<tr><td>{}</td><td>", what);
    match (code, OUT_DONE.load(Ordering::Relaxed)) {
        (0, false) => { let _ = write!(w, "<i>still trying</i>"); }
        (0, true) => { let _ = write!(w, "<b class=no>no answer</b>"); }
        (c, _) => { let _ = write!(w, "<b class=yes>{}</b>", c); }
    }
    let _ = write!(w, "</td></tr>");
}
...
table{{border-collapse:collapse;margin:0 0 1rem;font:13px/1.6 ui-monospace,monospace}}\
td{{padding:.15rem .8rem .15rem 0;border-bottom:1px solid #222}}\
.yes{{color:#8ae234}}.no{{color:#ef2929}}i{{color:#888}}\
...
this board's log.</p><table>",
        addr[0], addr[1], addr[2], addr[3],
        addr[0], addr[1], addr[2], addr[3], hi, lo, up,
        SERVED.load(Ordering::Relaxed)
    );

    // The result, above the log rather than inside it. This is the line
    // somebody photographs, and a reader should not have to find it among a
    // hundred others.
    outcome(&mut w, "GET http://1.1.1.1/", STATUS_ROOT.load(Ordering::Relaxed));
    outcome(
        &mut w,
        "GET http://cp.cloudflare.com/generate_204",
        STATUS_204.load(Ordering::Relaxed),
    );
    let resolved = RESOLVED.load(Ordering::Relaxed).to_be_bytes();
    let _ = write!(w, "<tr><td>{} resolves to</td><td>", OUT_NAME);
    match (RESOLVED.load(Ordering::Relaxed), OUT_DONE.load(Ordering::Relaxed)) {
        (0, false) => { let _ = write!(w, "<i>still trying</i>"); }
        (0, true) => { let _ = write!(w, "<b class=no>nothing</b>"); }
        _ => {
            let _ = write!(
                w, "<b class=yes>{}.{}.{}.{}</b>",
...
/// The name this board puts in its DHCP request, option 12.
///
/// exp151 and exp152 also *answered* to `<this>.local` over multicast DNS, and
/// that responder is not here. It is not a saving — it is a scope line. This
/// experiment is about what happens to a packet on its way out, and a name that
/// a phone has already been measured never to ask for would be a second story
/// in a firmware that is meant to tell one.
const HOSTNAME: &str = "yi26";
...
/// The status code off the front of a response, and nothing else parsed.
///
/// A firmware that parses a reply it did not ask for is a firmware doing work
/// on untrusted input; the three digits after the first space are the whole of
/// what this experiment claims to read.
fn status_of(head: &[u8]) -> Option<u32> {
...
/// The first line, without its terminator, for putting in the log verbatim.
fn first_line(buf: &[u8]) -> &[u8] {
    let end = buf.iter().position(|&b| b == b'\r' || b == b'\n').unwrap_or(buf.len());
    &buf[..end]
}

/// The value of one header, if it is inside what was kept.
///
/// Case-sensitive and deliberately so: this looks for exactly what the server
/// under test sends, and a match found by being lenient would be a match that
/// says less about what actually arrived.
fn header<'a>(buf: &'a [u8], name: &[u8]) -> Option<&'a [u8]> {
    let mut i = 0;
    while i + name.len() <= buf.len() {
        if buf[i..].starts_with(name) && (i == 0 || buf[i - 1] == b'\n') {
            let rest = &buf[i + name.len()..];
            let start = rest.iter().position(|&b| b != b' ')?;
            return Some(first_line(&rest[start..]));
        }
        i += 1;
    }
    None
...
/// One connection, one request, one status code.
///
/// The socket is built and dropped inside here so a failed attempt leaves
/// nothing behind for the next one to inherit — a half-open socket reused
/// across attempts would make the second attempt a different experiment from
/// the first.
async fn fetch_once(
    ...
    socket.set_timeout(Some(OUT_TIMEOUT));

    let began = Instant::now();
    let remote = (Ipv4Address::new(OUT_HOST[0], OUT_HOST[1], OUT_HOST[2], OUT_HOST[3]), OUT_PORT);

    match with_timeout(OUT_TIMEOUT, socket.connect(remote)).await {
        Err(_) => {
            log!("out: {} — connect timed out after {} s", label, OUT_TIMEOUT.as_secs());
            return None;
        }
        Ok(Err(e)) => {
            log!("out: {} — connect refused ({:?})", label, e);
            return None;
        }
        Ok(Ok(())) => {}
    }
    let connected = (Instant::now() - began).as_millis();
    log!("out: {} — connected in {} ms", label, connected);

    if socket.write_all(request.as_bytes()).await.is_err() || socket.flush().await.is_err() {
        log!("out: {} — the request did not go", label);
        socket.abort();
        return None;
    }

    // The head is kept and the rest is only counted. A redirect's body is a
    // courtesy page nobody here reads, and keeping it would mean carrying a
    // buffer sized by what a stranger decides to send.
    let mut head = [0u8; 256];
    let mut kept = 0usize;
    let mut total = 0usize;
    let mut scratch = [0u8; 512];
    loop {
        match with_timeout(OUT_TIMEOUT, socket.read(&mut scratch)).await {
            Err(_) => {
                log!("out: {} — the answer stopped mid-way after {} bytes", label, total);
                break;
            }
            Ok(Ok(0)) | Ok(Err(_)) => break,
            Ok(Ok(n)) => {
                total += n;
                let room = head.len() - kept;
                let take = core::cmp::min(room, n);
                head[kept..kept + take].copy_from_slice(&scratch[..take]);
                kept += take;
            }
        }
    }
    ...
    let ms = (Instant::now() - began).as_millis();
    if kept == 0 {
        log!("out: {} — connected and got nothing back", label);
        return None;
    }

    if let Ok(line) = core::str::from_utf8(first_line(&head[..kept])) {
        log!("out: {} — {}", label, line);
    }
    // Printed because it is the finding rather than a detail: the answer to a
    // plain request names a protocol this board cannot speak.
    if let Some(loc) = header(&head[..kept], b"Location:") {
        if let Ok(loc) = core::str::from_utf8(loc) {
            log!("        Location: {}", loc);
        }
    }
    let code = status_of(&head[..kept]);
    log!(
        "        {} bytes in {} ms",
        total, ms
    );
...
/// The experiment. Runs once the board has an address, reports, and stops.
///
/// It stops rather than repeating because a result that keeps being re-measured
/// fills the log with the reader's own waiting — the rule
/// `docs/debugging-on-a-phone.md` calls *the reader's own footsteps*. The three
/// attempts are for a transient failure, not for a watcher.
#[embassy_executor::task]
async fn out_task(
    stack: embassy_net::Stack<'static>,
    rx: &'static mut [u8],
    ...
    if gateway.is_none() {
        // Not an attempt that failed — an attempt that was never worth making,
        // and saying so is the difference between "there is no way out" and
        // "the way out did not work".
        log!("out: no gateway in the lease, so there is nothing to try. Nothing was sent.");
        OUT_DONE.store(true, Ordering::Relaxed);
        core::future::pending::<()>().await;
        unreachable!()
    }

    for attempt in 1..=OUT_ATTEMPTS {
        if attempt > 1 {
            log!("out: attempt {} of {}", attempt, OUT_ATTEMPTS);
        }

        if STATUS_ROOT.load(Ordering::Relaxed) == 0 {
            if let Some(code) = fetch_once(stack, rx, tx, "GET http://1.1.1.1/", OUT_ROOT).await {
                STATUS_ROOT.store(code, Ordering::Relaxed);
            }
        }
        if STATUS_204.load(Ordering::Relaxed) == 0 {
            if let Some(code) =
                fetch_once(stack, rx, tx, "GET http://cp.cloudflare.com/generate_204", OUT_204).await
            {
                STATUS_204.store(code, Ordering::Relaxed);
            }
        }

        if STATUS_ROOT.load(Ordering::Relaxed) != 0 && STATUS_204.load(Ordering::Relaxed) != 0 {
            break;
        }
        if attempt < OUT_ATTEMPTS {
            Timer::after(OUT_RETRY_GAP).await;
        }
    }

    // Third measurement, and last on purpose. It is the one that can fail on its
    // own terms, and running it after the other two means a resolver that is
    // missing cannot be mistaken for a link that is dead.
    match with_timeout(
        OUT_TIMEOUT,
        stack.dns_query(OUT_NAME, embassy_net::dns::DnsQueryType::A),
    )
    .await
    {
        Ok(Ok(answers)) => match answers.first() {
            Some(embassy_net::IpAddress::Ipv4(a)) => {
                let o = a.octets();
                RESOLVED.store(u32::from_be_bytes(o), Ordering::Relaxed);
                log!("dns: {} is {}.{}.{}.{}", OUT_NAME, o[0], o[1], o[2], o[3]);
            }
            _ => log!("dns: {} answered with nothing usable", OUT_NAME),
        },
        Ok(Err(e)) => log!("dns: {} did not resolve ({:?})", OUT_NAME, e),
        Err(_) => log!("dns: {} — no answer within {} s", OUT_NAME, OUT_TIMEOUT.as_secs()),
    }

    OUT_DONE.store(true, Ordering::Relaxed);
    log!("out: done. The page shows both answers; the difference between them is the point.");
...
async fn main(spawner: Spawner) {
    ...
    config.product = Some("exp153 out through the phone");
    config.serial_number = Some("153");
    ...
    // server.
    //
    // Seven sockets in use and one spare: the DHCP client's, the DNS resolver's
    // that comes with the `dns` feature, four for the HTTP workers, and **one
    // for the request that goes out** — which is the only one this experiment
    // added. Running out here is a panic rather than a refusal, so they are
    ...
    // The outbound pair. Smaller than a worker's, and sized by what is actually
    // kept: the response head is 256 bytes and everything past it is counted
    // and dropped.
    static OUT_RX: StaticCell<[u8; 1024]> = StaticCell::new();
    static OUT_TX: StaticCell<[u8; 512]> = StaticCell::new();

    spawner.spawn(lease_task(stack).unwrap());
    spawner.spawn(out_task(stack, OUT_RX.init([0; 1024]), OUT_TX.init([0; 512])).unwrap());
    spawner.spawn(report_task(stack).unwrap());

    log!("exp153 up. The same stack as exp152, pointed the other way.");
    ...
    // by the time they look.
    {
        ...
        log!("  then asking whether that address is good for anything beyond this cable:");
        log!("    GET http://1.1.1.1/ and GET http://cp.cloudflare.com/generate_204,");
        log!("    same address, one header different. The two answers are the experiment.");
...
const INQUIRY_PRODUCT: &[u8; 16] = b"exp153 going out";
...
<a href=\"http://{}.{}.{}.{}/\">http://{}.{}.{}.{}/</a>\
<p>The top of that page is the answer: two requests the board sent out through \
this phone, and what came back. One of them should be a redirect to a protocol \
the board cannot speak. That is the point, not a fault.</p>",
