...
//!
//! Full CTAP 2.1 PIN State Machine:
//! - setPIN (0x03), changePIN (0x04), getPinToken (0x05)
//! - 8 retries limit with lockout (0x34)
//! - pinUvAuthToken issuance and UV verification (FLAG_UV = 0x04)
...
include!(concat!(env!("OUT_DIR"), "/exp186_config.rs"));
...
const PRODUCT: &str = "exp186 the number behind the finger";
...
fn parse_get_assertion(body: &[u8]) -> Result<GetAssertion<'_>, u8> {
    ...
    Ok(GetAssertion { rp_id, client_data_hash, allow, n_allow, allow_truncated, pin_uv_auth_param })
}
...
fn parse_make_credential(body: &[u8]) -> Result<MakeCredential<'_>, u8> {
    ...
    Ok(MakeCredential { client_data_hash, rp_id, user_id, algs, n_algs, algs_truncated, pin_uv_auth_param })
}
...
fn decrypt_pin_payload(
    ...
    for (src_chunk, dst_chunk) in ciphertext.chunks_exact(16).zip(out.chunks_exact_mut(16)) {
        ...
        let mut block = *aes::Block::from_slice(src_chunk);
        cipher.decrypt_block(&mut block);
        for i in 0..16 {
            dst_chunk[i] = block[i] ^ prev[i];
        }
        prev = cur;
    }
    Ok(ciphertext.len())
                        ...
                        match ctap {
                                ...
                                match sub_cmd {
                                    ...
                                    Some(0x02) => {
                                        ...
                                        for _ in 0..10 {
                                            ...
                                            eph_bytes[0] &= 0x7f; // guarantee smaller than curve order
                                            eph_bytes[31] |= 0x01; // guarantee non-zero
                                    ...
                                    Some(0x03) => {
                                        ...
                                        let enc = match new_pin_enc {
                                            Some(e) => e,
                                            None => {
                                                log!("  setPIN: missing new_pin_enc");
                                                send(&mut writer, cid, CTAPHID_CBOR, &[CTAP2_ERR_MISSING_PARAMETER]).await;
                                                continue;
                                            }
                                        };
                                        ...
                                        let shared_secret = match decapsulate_shared_secret(&session_sk, &peer_x, &peer_y) {
                                            Ok(s) => s,
                                            Err(_) => {
                                                log!("  setPIN: decapsulate_shared_secret failed");
                                    ...
                                    Some(0x05) => {
                                        ...
                                        log!("  getPinToken: PIN verified, issued active pinUvAuthToken");

                                        out[0] = CTAP2_OK;
                                        let mut w = cbor::Writer::new(&mut out[1..]);
                                        w.map(1);
                                        w.key(0x02); // pinUvAuthToken (key 0x02 in getPinToken response)
                                        w.bytes(&token_enc);
                                        w.end();
                                        if let Ok(b) = w.finish() {
                                            let n = 1 + b.len();
                                    ...
                                    _ => {
                                        log!("  clientPIN: subCommand {:?} unsupported", sub_cmd);
                                        send(&mut writer, cid, CTAPHID_CBOR, &[CTAP2_ERR_UNSUPPORTED_OPTION]).await;
                                    }
                                }
                            }
                                    ...
                                    Ok(req) => {
                                        ...
                                        if let Some(param) = req.pin_uv_auth_param {
                                            if verify_pin_uv_auth_token(pin_state.active_token.as_ref(), req.client_data_hash, Some(param)) {
                                                ...
                                                log!("  makeCredential: pinUvAuthParam INVALID");
                                                send(&mut writer, cid, CTAPHID_CBOR, &[CTAP2_ERR_PIN_AUTH_INVALID]).await;
                                                continue;
                                            }
                                        }
                                        ...
                                        match built {
                                            Ok(b) => {
                                                resp[0] = CTAP2_OK;
                                                log!(
                                                    "  credential created: authData {}B, total {}B (UV={})",
                                                    b.auth_len,
                                                    b.response,
                                                    user_verified
                                                );
                            ...
                            AUTHENTICATOR_GET_ASSERTION => {
                                ...
                                match parse_get_assertion(body) {
                                    Ok(req) => {
                                        log!("  getAssertion: rp={:?}, allow={}", req.rp_id, req.n_allow);
                                        Timer::after(PACE).await;
                                        ...
                                        for &entry in &req.allow[..req.n_allow] {
                                            if credential_is_ours(entry, &rp_id_hash) {
                                        ...
                                        if let Some(param) = req.pin_uv_auth_param {
                                            if verify_pin_uv_auth_token(pin_state.active_token.as_ref(), req.client_data_hash, Some(param)) {
                                                ...
                                                log!("  getAssertion: pinUvAuthParam INVALID");
                                                send(&mut writer, cid, CTAPHID_CBOR, &[CTAP2_ERR_PIN_AUTH_INVALID]).await;
                                                continue;
                                            }
                                        }
                                        ...
                                        let mut resp = [0u8; 512];
                                        match build_assertion(&mut resp[1..], req.rp_id, cred_id, req.client_data_hash, present, user_verified) {
...
async fn main(spawner: Spawner) {
    ...
    log!("exp186 the client that must know");
    Timer::after(PACE).await;
