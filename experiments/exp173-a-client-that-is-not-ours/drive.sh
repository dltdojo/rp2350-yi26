...
#
# The seven `ga-` cases are the experiment. Each one registers a fresh
# credential first, so the round trip is a round trip and not a replay of
# something recorded earlier. Six of them are shapes a
...
CASES=("init" "ping 58" "ping 1024" "ping 2000" "bad-seq" "busy" "truncated"
       ...
       "ga-roundtrip" "ga-forged" "ga-other-rp" "ga-wrong-length"
       "ga-empty-allow" "ga-no-allow" "ga-decoys")
...
sleep 2
yi26 pflash target/exp172-none.uf2 > /dev/null 2>&1
