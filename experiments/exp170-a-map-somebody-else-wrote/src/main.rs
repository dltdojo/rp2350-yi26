...
/// exp169 measured that an empty `versions` array is accepted by `libfido2` and
/// that claiming `FIDO_2_0` gets a tool to `FIDO_ERR_INTERNAL`. This device
/// still cannot make a credential, so it still claims nothing — the honest
/// build of that pair, carried forward rather than re-argued.
const VERSIONS: [&str; 0] = [];
...
const PRODUCT: &str = "exp170 a map somebody else wrote";
...
fn find_text_key(r: &mut Reader, pairs: u64, want: &str) -> Result<bool, ReadError> {
    ...
    for _ in 0..pairs {
        ...
        if is_it && !found {
            found = true;
            // The value is left unread: the caller reads it, then the loop
            // continues from wherever that left the cursor.
            return Ok(true);
        }
        r.skip()?;
    }
                    ...
                    CTAPHID_CBOR => {
                        ...
                        match ctap {
                            ...
                            AUTHENTICATOR_MAKE_CREDENTIAL => {
                                ...
                                match parse_make_credential(body) {
                                    Ok(req) => {
                                        ...
                                        ERRORS.fetch_add(1, Ordering::Relaxed);
                                        let status = if es256 {
                                            log!("  ES256 was offered, and this device still has no key to sign with.");
                                            CTAP2_ERR_OPERATION_DENIED
                                        } else {
                                            log!("  ES256 was not offered; nothing here could be used anyway.");
                                            CTAP2_ERR_UNSUPPORTED_ALGORITHM
                                        };
                                        Timer::after(PACE).await;
                                        log!("  refusing with {:#04x}, having understood the request.", status);
                                        Timer::after(PACE).await;
                            ...
                            AUTHENTICATOR_GET_ASSERTION => {
                                ERRORS.fetch_add(1, Ordering::Relaxed);
                                log!("  getAssertion: there is no credential here to assert.");
                                Timer::after(PACE).await;
...
async fn main(spawner: Spawner) {
    ...
    log!("exp170 a map somebody else wrote");
    Timer::after(PACE).await;
    ...
    Timer::after(PACE).await;
    log!("  CTAP2: getInfo, and makeCredential read but refused.");
    Timer::after(PACE).await;
    {
