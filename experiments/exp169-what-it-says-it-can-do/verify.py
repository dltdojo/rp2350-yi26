...
EXPECTED = {
    ...
    "makecred": ("ctap", "CTAP1_ERR_INVALID_COMMAND"),
...
def main():
    ...
    # The claim, and what a tool that believes it does. Both builds appear in a
    # full transcript, and the pair is the finding rather than either half.
    claims = [l.strip() for l in lines if "version strings:" in l]
    if claims:
        notes.append(f"a build that claims a version: {claims[0]}")
        if not any("FIDO_ERR" in l or "fido2-token:" in l for l in lines):
            problems.append("a version was claimed and nothing in this transcript tested it")
